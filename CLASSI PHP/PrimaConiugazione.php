...
class PrimaConiugazione implements Coniugazione {
    ...
    private $desinenza_indicativo_presente = ["o","i","a","iamo","ate","ano"];
    private $desinenza_indicativo_imperfetto = ["avo","avi","ava","avamo","avate","avano"];
    private $desinenza_indicativo_passato_remoto = ["ai","asti","ò","ammo","aste","arono"];
    private $desinenza_indicativo_futuro_semplice = ["erò","erai","erà","eremo","erete","eranno"];
    private $desinenza_congiuntivo_presente = ["i","i","i","iamo","iate","ino"];
    private $desinenza_congiuntivo_imperfetto = ["assi","assi","asse","assimo","aste","assero"];
    private $desinenza_condizionale_presente = ["erei","eresti","erebbe","eremmo","ereste","erebbero"];
    private $desinenza_imperativo_presente = ["","a","i","iamo","ate","ino"];
    private $desinenza_participio_presente = "ante";
    private $desinenza_participio_passato = "ato";
    private $desinenza_gerundio_presente = "ando";
    ...
    private function stampa_desinenza_del_verbo($array_desinenza, &$output_string){
        ...
        if ( (substr($stringa, -4) === "care") || (substr($stringa, -4) === "gare") ) {
            //verbi -care o -gare
            $this->eccezioneCareGare($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);

            //ESCO DALLA FUNZIONE
            return;
        
        }else if (substr($stringa, -4) === "iare") {
        //verbi -iare (ma difficili da gestire, bisogna vedere se la i è accentata)
        //non lo tratto per ora
            if (substr($stringa, -6) === "sciare") {
            //verbo sciare (eccezione dei verbi -ciare e -giare)
            // (usare funzione coniugazione classica)
                $this->coniugazione_verbo_no_eccezioni($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);

                //ESCO DALLA FUNZIONE
                return;
            }else{
                if (substr($stringa, -5) === "ciare" || substr($stringa, -5) === "giare") {
                    $this->eccezioneCiareGiare($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);
                    return;
                }
            }
            
            if (substr($stringa, -6) === "gliare") {
                //verbi -gliare
                $this->eccezioneGliare($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);
                //ESCO DALLA FUNZIONE
                return;
            }

            //caso semplice -iare, non lo tratto correttamente
            $this->coniugazione_verbo_no_eccezioni($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);
        }else if(substr($stringa, -4) === "eare") {
        //verbi -eare (raddoppio e, usare funzione coniugazione classica)
            $this->coniugazione_verbo_no_eccezioni($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);
        }else{
        //verbi senza eccezioni, coniugo semplicemente
            $this->coniugazione_verbo_no_eccezioni($stringa, $lunghezza_radice_parola, $array_desinenza, $output_string);
        }
    ...
    private function eccezioneCareGare(string $stringa, int $lunghezza_radice_parola, array $array_desinenza, string &$output_string) : void {
        for ($i=0; $i < self::numero_pronomi; $i++) {
            //stampo radice del verbo
            //salvo eventuale stampa in una stringa
            //così potenzialmente o la ritorno, o la stampo
            ...
            if($array_desinenza[$i] != "") // per prima persona dell'imperativo presente
                //$output_string .= substr($stringa, 0, $lunghezza_radice_parola);
                $output_string .= $this->verbo->getRadice();
            ...
            if($array_desinenza[$i] == ""){
                ...
                $output_string .= "-";
                $output_string .= "</span>";
            }else if( $this->prima_lettera($array_desinenza[$i]) == 'i' || $this->prima_lettera($array_desinenza[$i]) == 'e'){
            //se primo carattere desinenza uguale a 'i' o 'e'
            //aggiungo h
                $output_string .= '<span style="color: red;">' . 'h' . $array_desinenza[$i] . '</span>';
                $output_string .= "</span>";
            }else{
                $output_string .= '<span style="color: red;">' . $array_desinenza[$i] . '</span>';
                $output_string .= "</span>";
    ...
    private function eccezioneCiareGiare(string $stringa, int $lunghezza_radice_parola, array $array_desinenza, string &$output_string) : void {
        for ($i=0; $i < self::numero_pronomi; $i++) {
            //stampo radice del verbo
            //salvo eventuale stampa in una stringa
            //così potenzialmente o la ritorno, o la stampo
    ...
    private function eccezioneGliare(string $stringa, int $lunghezza_radice_parola, array $array_desinenza, string &$output_string) : void {
        for ($i=0; $i < self::numero_pronomi; $i++) {
            //stampo radice del verbo
            //salvo eventuale stampa in una stringa
            //così potenzialmente o la ritorno, o la stampo
